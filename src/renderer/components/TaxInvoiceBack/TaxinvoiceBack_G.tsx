...
function TaxinvoiceBack_G() {
  ...
  const handelSubmit = (event: FormEvent) => {
    ...
    if (button === 1) {
      ...
      navigate(`/${CREATE_INVOICE_GOLD}/${VIEW_INVOICE_GOLD}`);
    } else if (button === 2) {
      // @ts-ignore
      detail.slno = ino.Estd + 1;
      taxStore.setInvoiceDetail?.(detail);
      navigate(`/${CREATE_INVOICE_GOLD}/${VIEW_ESTEEMED_GOLD}`);
    } else {
      // @ts-ignore
      detail.slno = ino.BS + 1;
      taxStore.setInvoiceDetail?.(detail);
      navigate(`/${CREATE_INVOICE_GOLD}/${VIEW_BOS_GOLD}`);
    }
  };
...
export default TaxinvoiceBack_G;
