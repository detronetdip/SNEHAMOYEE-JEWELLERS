...
import { BACK_SILVER, CREATE_INVOICE_SILVER } from '../../path/index';

const Invoice_GOLD = React.forwardRef((_props, _ref) => {
  ...
  invoiceDetail?.products.forEach((element: any) => {
    ...
    VOS += element.wt * (invoiceDetail.rate / 10) + element.mc;
  });
  ...
  return (
    <>
      ...
      <ReactToPrint
        ...
        onAfterPrint={askToSave}
        content={() => refer.current}
      />
      <button
        type="button"
        onClick={() => navigate(`/${CREATE_INVOICE_SILVER}/${BACK_SILVER}`)}
      >
        Back
      </button>
    </>
