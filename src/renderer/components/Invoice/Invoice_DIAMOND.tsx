...
import { Taxstore } from '../../types/index';

const Invoice_DIAMOND = React.forwardRef((_props, _ref) => {
  ...
  invoiceDetail?.products.forEach((element: any) => {
    ...
    VOS +=
      +parseFloat(
        `${element.wt * (invoiceDetail.rate / 10) + element.mc * element.wt}`
      ).toFixed(2) + +element.dm_price.toFixed(2);
  });
  console.log(VOS);
...
export default Invoice_DIAMOND;
